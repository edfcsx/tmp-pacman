...
Scene::~Scene()
{
	// remove da memoria os objetos estáticos
	for (auto obj : statics)
		delete obj;

	// remove da memoria os objetos em movimento
	for (auto obj : moving)
		delete obj;
}
...
{
	// se as coordendas inteiras são iguais
...
{
	// se as coordendas do ponto estão dentro do retângulo
...
{
	// se a distância entre o ponto e o centro do circulo
...
bool Scene::Collision(Mixed* m, Geometry* s)
{
	...
	// percorra lista até achar uma colisçao
	...
	while (!collision && i != m->shapes.end())
	{
		...
		// passa para a próximo geometria da forma mista
		++i;
	}
