/*******************************************************************************
	...
	Descrição:	Define uma classe para gerenciar o cenário do jogo.
				...
				Ele pode ser usado também para percorrer a lista de objetos
				para realizar algum processamento individual nos objetos.

				A classe usa List para organizar os objetos A list ela é
				uma estrutura de dados que se conecta através de ponteiros
				sendo it e next ponteiros, ao realizar a exclusão 
				de algum elemento a list reconecta todos os pontos novamente
				sendo mais rapida que um vetor.

				Jogos profissionais alocam toda a memoria de uma unica vez
				assim evitam perder perfomance com essas operações.
*******************************************************************************/
...
using std::list;								// acessa lista sem precisar do std::
...
enum ObjectGroup
{
	...
	MOVING										// colidem entre di e com estáticos
};
...
private:
	...
	bool Collision(Point * p, Rect * r);		// colisão entre ponto e retânculo
	bool Collision(Point * p, Circle * c);		// colisão entre ponto e círculo

	bool Collision(Rect * ra, Rect * rb);		// colisão entre retângulo e retângulo
	bool Collision(Rect * r, Point * p);		// colisão entre retângulo e ponto
	bool Collision(Rect * r, Circle * c);		// colisão entre retângulo e círculo
												
	bool Collision(Circle * ca, Circle * cb);	// colisão entre círculo e círculo
	bool Collision(Circle * c, Point * p);		// colisão entre círculo e ponto
	bool Collision(Circle * c, Rect * r);		// colisão entre círculo e retângulo
...
public:
	...
	void Add(Object * obj, int type);			// adiciona um objeto na lista STATIC ou MOVING da cena
	...
	void Delete();								// deleta objeto cuso Update/Draw está sendo executado
