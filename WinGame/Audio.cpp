...
Audio::Audio()
{
	...
	{
		// cria uma instância de engine Xaudio2
...
Audio::~Audio()
{
	// delete todos os sons da coleção
	...
	// libera a instância do XAudio2
	if (audioEngine) audioEngine->Release();

	// Xaudio 2 usa a biblioteca COM
	CoUninitialize();
}
...
void Audio::Play(uint id, bool repeat)
{
	...
	// limpa a voz corrente e inicia reprodução do som
