/*******************************************************************************
	...
	Descrição:	Agrupa a definição de todas as formas geométricas suportadas:
				ponto, linha, retânculo, circulo, polígno e mista (agrupamento
...
{
	// cria uma linha de (x1, y1) até (x2, y2)
	a = Point(x1, y1);
	b = Point(x2, y2);
...
Poly::Poly()
{
	vertexCount = 0;			// poligono não tem vértices
...
{
	// guarda o número de vértices do polígono
	vertexCount = vCount;

	// aloca mémoria para os vértices
...
Mixed::~Mixed()
{
	// libera memória ocupa pelas geometrias que compões essa forma mista
	for (auto i : shapes)
		delete i;
}
